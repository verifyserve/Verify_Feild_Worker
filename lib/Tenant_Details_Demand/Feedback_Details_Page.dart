...
class _Feedback_DetailsState extends State<Feedback_Details> {
  ...
  void _showBottomSheet(BuildContext context, Subid) {
            ...
            return GestureDetector(
              onTap: () async {
                ...
                final responce = await http.get(Uri.parse
                  ('https://verifyserve.social/WebService4.asmx/add_under_feedback_tenant_demand_?feedback=${names[index]}&subid=$Subid&addtional_info='+'Date = '+formattedDate+' | Time = '+formattedTime));
                //final responce = await http.get(Uri.parse('https://verifyserve.social/WebService2.asmx/Add_Tenants_Documaintation?Tenant_Name=gjhgjg&Tenant_Rented_Amount=entamount&Tenant_Rented_Date=entdat&About_tenant=bout&Tenant_Number=enentnum&Tenant_Email=enentemail&Tenant_WorkProfile=nantwor&Tenant_Members=enentmember&Owner_Name=wnername&Owner_Number=umb&Owner_Email=emi&Subid=3'));

                if(responce.statusCode == 200){
                  print(responce.body);
                  Navigator.of(context).pushAndRemoveUntil(MaterialPageRoute(builder: (context) => Feedback_Details(id: '${Subid}',),), (route) => route.isFirst);
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            ...
            FutureBuilder<List<Catid_review>>(
                future: fetchData_review(widget.id),
                builder: (context,abc){
                  if(abc.connectionState == ConnectionState.waiting){
                    return Center(child: CircularProgressIndicator());
                  }
